...
class UploadeController extends Controller {
    ...
    public function uploade(){
        ...
        if(is_uploaded_file($_FILES['fileNames']['tmp_name'])){//判断有没有上传服务端储存临时文件名
            ...
            //$chinaName = iconv("utf-8", "gb2312", $name);//解决不能上传中文名称文件的问题
            $chinaName = $name;
    ...
    public function delFile()
    {
        if(IS_AJAX)
        {
            ...
            /**
             * 这里有两种方法（上传时用哪种，下载时就用哪种，同样的删除也用哪种，不然会提示文件找不到）
             * 1，解决上传到服务器的文件中文名字正常显示的问题，但是这样下载时，下载的名字就会已乱码形式显示
             * 2，上传到服务器的文件中文名字以乱码形式显示，但是用户下载时，显示的是正常的中文名字
             * @综合用户体验度考虑，就已第2种方法去实现。而且服务器里的文件一般也不会去看，想看的话也可以通过下载来下载到本地查看 **/
            //$newFileName = iconv("utf-8","gb2312",$filename);//解决中文不能显示出来的问题
    ...
    public function uploadeVideo(){
        ...
        //if(is_uploaded_file($_FILES['fileNames']['tmp_name'])){//判断有没有上传服务端储存临时文件名
            ...
            if(!$this->videoIsOk($suffix))
            {
                $this->errmsg(10001, '只允许上传mp4的文件');
            }
            ...
            if($error == 0)
            {
                $add['userId']     = $adminId;//上传者id
                $add['type']       = 1;//上传者类型  1是管理员
                $add['videoUrl']   = $fileUPloadUrl;//文件所在路径
                $add['size']       = $size;//文件大小
                $add['remark']     = $fileUploadName;//文件名称
                $add['createDate'] = date('Y-m-d H:i:s', time());
                $res = M('video')->add($add);
                if(!$res)
                {
    ...
    /**
     *判断文件后缀是否是允许上传的
     *@param string $suffix :带点的后缀名(如.mp4)
     *@return bool 如果是允许上传的后缀名返回true,否则返回false
     */
    private function videoIsOk($suffix){
        $suffix_arr = array('.mp4');
        return in_array(strtolower($suffix), $suffix_arr);//strtolower把所有字符转换成小写
    }
}